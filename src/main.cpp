...
uint256 hashGenesisBlock("0x3dcee5acc0abf9f47e695cf83371e72afb5eacc0190c2f6798b80409ad9d0675");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Giarcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Giarcoin Signed Message:\n";
...
{
    // Giarcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
     ...
     if (nHeight >=1 && nHeight <= 200)  // KGW kicks in at block 180 before that 5 coins per block
    {
    	nSubsidy = 5 * COIN;
    }
...
static const int64 nTargetSpacing = 1.0 * 60; // Giarcoin: 1.0 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Giarcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Giarcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc6196dfb2e6569ee11501198332e0bba3a99ffe0bdec45ddb6a90ccc48079808");
    }
 ...
 */
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "3/30/2014 Hope to create a new merkel hash and genesis block";
        ...
        block.nVersion = 1;
        block.nTime    = 1396148400; //final time
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 10846792;

        if (fTestNet)
        {
            block.nTime    = 1396148400;
            block.nBits    = 0x1e0ffff0;
            block.nNonce   = 10846792;
        }
        ...
        if (fTestNet)
        {
            assert(block.hashMerkleRoot == uint256("0x6893164e54af0e812160ddacb8164d01480d32a8f2e0fa07771c67e6060738bf"));
        }
        else
        {
            assert(block.hashMerkleRoot == uint256("0x6893164e54af0e812160ddacb8164d01480d32a8f2e0fa07771c67e6060738bf"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Giarcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GiarcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GiarcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GiarcoinMiner(CWallet *pwallet)
{
    printf("GiarcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("giarcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GiarcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GiarcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GiarcoinMiner, pwallet));
}
